...
public class RunGame implements Iterable<Player> {
    ...
    void playGame() {
        ...
        while (playersInGame.size()>1 && round<gameLimit){
            ...
            byte winner = new Clash().resolveClash(playersInGame,activePlayers, cards, cardsForWinner);

            addCardsToWinner(this.cards, cardsForWinner, winner);
    ...
    List addCardsToWinner(List<List<Card>> cards, Cards listOfCards,byte winner){
        for(Card c: listOfCards){
            cards.get(winner-1).add(c);
        }
        return cards;
    }
